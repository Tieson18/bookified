...
export const getVoice = (persona?: string) => {
  const voices = voiceOptions.flatMap((group) => group.voices);
  const defaultVoice = voices.find((voice) => voice.id === DEFAULT_VOICE);

  if (!persona) return defaultVoice;

  // Find by voice ID
  const voiceEntry = voices.find((voice) => voice.id === persona);
